            ...
            if (loop_end != -1) {
                ...
                for (int j = 0; j < unroll_factor; j++) { // Repeat the loop body 'unroll_factor' times
                    for (int k = i + 1; k < loop_end; k++) {
                        fprintf(output_file, "%s\n", instructions[k]); // Write the loop body to the output file
                    }
                }

                // Write the original branch instruction
                fprintf(output_file, "%s\n", instructions[loop_end]);

                // Move the index to the branch instruction to avoid repeating the branch
                i = loop_end; // Update to the last instruction of the loop
            }
        }
    }
}
...
int main() {
    ...
    int unroll_factor = 2; // Set the desired unroll factor
